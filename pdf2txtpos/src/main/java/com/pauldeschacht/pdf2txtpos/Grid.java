...
public class Grid 
{
    ...
    public String[][] csv(List<WordPosition> words) {
        return csv(words,false);
    }
    public String[][] csv(List<WordPosition> words, boolean bVerticalOnly) {

        int numberRows = _ys.length;
        if (bVerticalOnly == true) {
            //use lines to 
            int maxLine = -1;
            for(WordPosition word: words) {
                maxLine = java.lang.Math.max(maxLine, word.getLineNb());
            }
            numberRows = maxLine;
        }
	String[][] csv = new String[numberRows][_xs.length];
	...
	for(WordPosition word: words) {
	    Point index = findGridIndex(word, bVerticalOnly);
	    if(index!=null) {
		...
		if (row>=0 && row<numberRows && col>=0 && col<_xs.length) {
    ...
    public Point findGridIndex(WordPosition word) {
        return findGridIndex(word, false);
    }
    public Point findGridIndex(WordPosition word, boolean verticalOnly) {
	float wx1 = word.x1();
	float wy1 = word.y1();
	float wx2 = word.x2();
	float wy2 = word.y2();
